 ...
 *
 * Fuente: División Político Administrativa (DIVIPOLA) del DANE.
 * Cubre los 32 departamentos + Bogotá D.C. con sus municipios más
 * poblados (no exhaustivo, pero cubre >95% del territorio relevante
 * para e-commerce).
 *
 * Si necesitás agregar un municipio que falta, editá la lista del
 * departamento correspondiente — están en orden alfabético dentro
 * de cada uno.
 */

export const UBICACIONES_COLOMBIA = {
  'Amazonas': [
    'Leticia', 'Puerto Nariño',
  ],
  'Antioquia': [
    'Abejorral', 'Amagá', 'Andes', 'Apartadó', 'Bello', 'Caldas', 'Carepa',
    'Caucasia', 'Chigorodó', 'Ciudad Bolívar', 'Copacabana', 'El Bagre',
    'El Carmen de Viboral', 'El Retiro', 'El Santuario', 'Envigado',
    'Frontino', 'Girardota', 'Guarne', 'Itagüí', 'Jardín', 'La Ceja',
    'La Estrella', 'La Pintada', 'Marinilla', 'Medellín', 'Necoclí',
    'Puerto Berrío', 'Rionegro', 'Sabaneta', 'San Pedro de los Milagros',
    'Santa Fe de Antioquia', 'Santo Domingo', 'Sonsón', 'Támesis',
    'Turbo', 'Urrao', 'Yarumal',
  ],
  'Arauca': [
  ...
  'Atlántico': [
    'Baranoa', 'Barranquilla', 'Galapa', 'Juan de Acosta', 'Luruaco',
    'Malambo', 'Manatí', 'Palmar de Varela', 'Piojó', 'Polonuevo',
    'Ponedera', 'Puerto Colombia', 'Repelón', 'Sabanagrande', 'Sabanalarga',
  ...
  'Bolívar': [
    ...
    'El Carmen de Bolívar', 'El Guamo', 'Magangué', 'Mahates', 'Margarita',
    'María la Baja', 'Mompox', 'Morales', 'Pinillos', 'Regidor', 'Río Viejo',
    'San Cristóbal', 'San Estanislao', 'San Fernando', 'San Jacinto',
    'San Juan Nepomuceno', 'San Martín de Loba', 'San Pablo', 'Santa Catalina',
    'Santa Rosa', 'Santa Rosa del Sur', 'Simití', 'Soplaviento', 'Talaigua Nuevo',
    'Tiquisio', 'Turbaco', 'Turbaná', 'Villanueva', 'Zambrano',
  ],
  'Boyacá': [
    'Aquitania', 'Belén', 'Boavita', 'Chiquinquirá', 'Combita', 'Cucaita',
    'Duitama', 'El Cocuy', 'Garagoa', 'Guateque', 'Jenesano', 'Maripí',
    'Miraflores', 'Moniquirá', 'Muzo', 'Nobsa', 'Otanche', 'Paipa', 'Pauna',
    'Paz de Río', 'Puerto Boyacá', 'Ramiriquí', 'Ráquira', 'Saboyá',
    'Samacá', 'San Mateo', 'Santa Rosa de Viterbo', 'Sogamoso', 'Soracá',
    'Sotaquirá', 'Tibasosa', 'Toca', 'Tota', 'Tunja', 'Tuta', 'Ventaquemada',
    'Villa de Leyva', 'Zetaquira',
  ],
  'Caldas': [
  ...
  'Chocó': [
    ...
    'Belén de Bajirá', 'Bojayá', 'Cantón del San Pablo', 'Carmen del Darién',
  ...
  'Cundinamarca': [
    ...
    'Cáqueza', 'Carmen de Carupa', 'Chaguaní', 'Chía', 'Chipaque',
  ...
  'Santander': [
    ...
    'Girón', 'Guaca', 'Guadalupe', 'Guapotá', 'Guavatá', 'Güepsa', 'Hato',
    'Jesús María', 'Jordán', 'La Belleza', 'La Paz', 'Landázuri', 'Lebrija',
    ...
    'Rionegro', 'Sabana de Torres', 'San Andrés', 'San Benito', 'San Gil',
  ...
  'Sucre': [
    ...
    'Morroa', 'Ovejas', 'Palmito', 'Sampués', 'San Antonio de Palmito',
    'San Benito Abad', 'San Juan de Betulia', 'San Luis de Sincé', 'San Marcos',
    'San Onofre', 'San Pedro', 'Santiago de Tolú', 'Sincelejo', 'Sucre',
    'Tolú Viejo',
  ],
  'Tolima': [
  ...
  'Valle del Cauca': [
    ...
    'Roldanillo', 'San Pedro', 'Sevilla', 'Toro', 'Trujillo', 'Tuluá', 'Ulloa',
...
export type Departamento = keyof typeof UBICACIONES_COLOMBIA

/** Lista alfabética de los 33 departamentos (incluye Bogotá D.C.) */
export const DEPARTAMENTOS: readonly Departamento[] = Object.keys(
  UBICACIONES_COLOMBIA,
).sort() as Departamento[]

/** Devuelve los municipios de un departamento (vacío si no existe) */
export function ciudadesDe(departamento: string): readonly string[] {
  const key = departamento as Departamento
  return UBICACIONES_COLOMBIA[key] ?? []
}

/** True si el par departamento/ciudad existe en el listado oficial */
export function esUbicacionValida(departamento: string, ciudad: string): boolean {
  return ciudadesDe(departamento).includes(ciudad)
}
